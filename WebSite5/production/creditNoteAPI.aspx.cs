...
public partial class WebSite5_production_creditNoteAPI : System.Web.UI.Page
{
    ...
    public void creditNoteApi()
    {
        ...
        try
        {
            ...
            string fileName = "";
            string contractNo = "";
            SqlDataReader readerload = API.getAllVpMemberNo();
            if (readerload.HasRows)
            {
                while (readerload.Read())
                {
                    memberNumber = memberNumber + readerload.GetString(0) + ",";
                    fileName = fileName + readerload.GetString(1) + ",";
                    contractNo = contractNo + readerload.GetString(2) + ",";
                }
                readerload.Close();
                ...
                for (int j = 0; j < contractNumberArray.Length - 1; j++)
                {
                    ...
                    WebRequest webReq = (HttpWebRequest)WebRequest.Create("https://karma-test.viewpointweb.com/v1/member/" + contractNumberArray[j] + "/contracts");
                    webReq.Headers.Add("api_key", "db4ba1fd-5d20-4e10-9b1e-b1d34c46d524");
                    WebResponse response = webReq.GetResponse();
                    ...
                    for(int k = 0; k < recordCount; k++)
                    {
                        ...
                        if (contractNoArray[j] == vpcontractNo)
                        {
                            string query = "update [creditNote_VPMem] set contractid='" + contractID + "' where contractno='" + vpcontractNo + "' and VPmem='" + memberNo + "' ;";
                            SqlCommand cmd = new SqlCommand(query, sqlcon);
                            cmd.ExecuteNonQuery();
                            string[] files = Directory.GetFiles(@"\\192.168.10.40\D$\MC", "*" + fileNameArray[j] + "*", SearchOption.AllDirectories);
                            ...
                            foreach (string file in files)
                            {
                                ...
                                WebRequest postwebReq = (HttpWebRequest)WebRequest.Create("https://karma-test.viewpointweb.com/v1/contract/" + contractID + "/document");
                                ...
                                string query1 = "update [creditNote_VPMem] set Status='Inactive' where contractid='" + contractID + "' and VPmem='" + memberNo + "' ;";
                                SqlCommand cmd1 = new SqlCommand(query1, sqlcon);
                                cmd1.ExecuteNonQuery();
        ...
        catch (Exception ex)
        {
           ...
           // int failedDetails = API.InsertFailedDetails(failedContractID, failedSourceFileName, failedDescription, failedDocumentDate, failedDocumentFileType, failedDocumentSourceType, failedDocumentType, "", "");
            Response.AppendHeader("Refresh", "30;url=creditNoteAPI.aspx");
        ...
        try
        {
            ...
            if (readerload.HasRows)
            {
                ...
                for (int j = 0; j < contractNumberArray.Length - 1; j++)
                {
                    ...
                    WebRequest webReq = (HttpWebRequest)WebRequest.Create("https://karma.viewpointweb.com/v1/member/" + contractNumberArray[j] + "/contracts/?activeOnly=true");
                    webReq.Headers.Add("api_key", "eb906bc1-77b6-4aa5-bb29-5fb5735e9dc6");
                    WebResponse response = webReq.GetResponse();
                    StreamReader reader = new StreamReader(response.GetResponseStream());
                    int updateContractInfoStatus = API.updateContractIndoStatus(contractNumberArray[j]);
                    ...
                    failedContractID = contractID;
                    string query = "update contractinfo set contractid='" + contractID + "', status='Inactive' where contractno='" + contractNo + "' and VPmem='" + memberNo + "' ;";
                    SqlCommand cmd = new SqlCommand(query, sqlcon);
                    cmd.ExecuteNonQuery();
                    sqlcon.Close();
                    reader.Close();
                    ...
                    SqlConnection sqlcon1 = new SqlConnection(conn);
                    sqlcon1.Open();
                    string query2 = "select distinct SUBSTRING(Cust_Serv_Member, 1, 4) as Cust_Serv_Member,Cont_Cont_Numb,contractid,Cust_Serv_File_Name 'SourceFileName' ,Cust_Serv_Desc 'Description',convert(varchar(10),Cust_Serv_Date,126) 'DocumentDate','10038' DocumentFileType,Log_Code_Desc,docsrcname, docsrcid 'DocumentSourceType',case when Cust_Serv_Desc like '%receipt%' or Cust_Serv_Desc like '%cpa%'  then '94970' when Cust_Serv_Desc like '%pa %' then '94917' else '10130' end as 'DocumentType',ls.ID as LogSourceID,'COMMUNICATION' as LogType,'CLOSED' as LogStatus,'' LogComment,'10154' UserLogType,'' as AssignedToUser,convert(varchar(10),Cust_Serv_Date,126) as UserDate,'' as DueDate from CuMe_Curr_Cust_Serv join cume_Curr_log_code on Log_Code_Code=Cust_Serv_Type join cume_curr_cont on Cont_Mem_Numb=Cust_Serv_Member join CuMe_Curr_Col_Fina on Col_Fina_Cont_Numb=Cont_Cont_Numb join [contractinfo] on [contractno]=Cont_Cont_Numb join [DOC Source type] on docsrcname=Log_Code_Desc join log_source ls on ls.DESCRIPTION=docsrcname where Cust_Serv_Date>=Cont_DSR_Date and (Cust_Serv_Desc not like '%pa %' or Cust_Serv_Desc  like '%cpa%') and contractid='" + contractID + "';";
                    ...
                    while (reader2.Read())
                    {
                        ...
                        for (int i = 0; i < array.Length - 1; i++)
                        {
                            ...
                            foreach (string file in files)
                            {
                                ...
                                {
                                    string query4 = "insert into collection_corres values('" + contractID + "','" + sourceFileName + "','" + Description + "','" + DocumentDate + "','" + DocumentFileType + "','" + DocumentSourceType + "','" + DocumentType + "','');";
                                    ...
                                    postwebReq.Method = "POST";
                                    postwebReq.ContentType = "application/json";
                                    string postData = "{\"flow_id\":\""+"5f69a18cd88dc038805b6795"+"\",\"sender\":\""+"KARMAS"+"\",\"recipients\":[{\"mobiles\":\""+"917709005437"+"\",\"VAR1\":\""+"##mobiles##"+"\",\"VAR2\":\""+"VALUE 2"+"\"]}";
                                    using (var streamWriter = new StreamWriter(postwebReq.GetRequestStream()))
                                    {
                                        streamWriter.Write(postData);
                                        streamWriter.Flush();
